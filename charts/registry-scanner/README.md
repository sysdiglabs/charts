...
| scanOnStart.enabled                  | true/false on whether to start the scanning after installation <br/>beware that concurrencyPolicy does not work on jobs (only cronjobs). <br/>always check if there is any scanning running and if so, remove it.            | <code>false</code>                                                  |
