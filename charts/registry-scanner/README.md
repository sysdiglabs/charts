...
| config.filter.maxAgeDays                            | Based on its creation date, excludes images older than the specified number of days.<br/>The maximum is 1825 (i.e., 5 years).                                                                                                              | <code>90</code>                               |
| config.filter.maxTagsPerRepository                  | Based on its creation date, newer ones take precedence, the maximum number of tags to scan per repository.<br/>The maximum is 100.                                                                                                         | <code>5</code>                                |
| config.filter.maxRepositoriesPerRegistry            | The number of repositories to scan per registry.<br/>The maximum is 20000.                                                                                                                                                                 | <code>500</code>                              |
| config.scan.inlineScanImage                         | Override the default image for the workers. If not set and any image field (registry, repository, tag) is configured, automatically uses the same image as the main orchestrator.                                                          | <code>""</code>                               |
...
| image.registry                                      | The Sysdig Registry Scanner image registry. This also sets the registry for the worker image unless explicitly overridden.                                                                                                                 | <code>quay.io</code>                          |
| image.repository                                    | The Sysdig Registry Scanner image repository. This also sets the repository for the worker image unless explicitly overridden.                                                                                                             | <code>sysdig/registry-scanner</code>          |
| image.tag                                           | The Registry Scanner image tag. If empty, default to appVersion in `Chart.yaml`. This also sets the tag for the worker image unless explicitly overridden.                                                                                 | <code></code>                                 |
...
| scanOnStart.jobName                                 | The name of the job created for the post-install scanner job                                                                                                                                                                               | <code>"registry-scanner-scan-on-start"</code> |
