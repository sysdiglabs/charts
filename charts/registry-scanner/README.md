...
| config.aws.managementAccountRoleARN  | In an AWS organizational custom setup, the aws role arn to have access to the management account.                                                                                                                            | <code></code>                                                       |
| config.aws.memberAccountsRoleName    | In an AWS organizational custom setup, the aws role attached only to the member accounts allowed to scan.                                                                                                                    | <code>"OrganizationAccountAccessRole"</code>                        |
| config.aws.allowListMemberAccountIDs | In an AWS organizational custom setup, the member accounts ids to scan. If empty, it will scan everything.                                                                                                                   | <code>""</code>                                                     |
